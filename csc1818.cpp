...
int main() {
    ...
    while(!game_over(s)){
        ...
        if(move == 'a'){
           ...
           if(change){ /// redability would be improved if you just checked this after checking every "a", "w", etc. movement
           insert2(s); /// worng indentation
           print_grid(s);
           }
       }
...
bool game_over(const std::vector<int>& v){
    ...
    /// this implementation works, but you shouldn't use the side effect of a function that primarily
    /// is used to perform a change (in this case the side effect the the boolean output of proc_num)
    /// if the aim is to check wether a certain property holds or not (in this case, if it's possible to keep
    /// playing or not)
    return !moves;
}
